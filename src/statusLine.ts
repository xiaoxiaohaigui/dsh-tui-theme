...
let prefCache: { at: number; mtimeMs: number; value: string | undefined } | undefined

/** -1 when the pref does not exist (a stable answer for the cache key). */
function prefMtimeMs(dataDir: string): number {
  try {
 ...
 */
export function invalidateThemePrefCacheForTests(): void {
  prefCache = undefined
  ...
  for (const dir of [themesTargetDir(), bundledThemesDir()]) {
    try {
      ...
      return {
        ...
        // The clock/turns are secondary garnish, not body text: the rich view
        // made the old body-text choice read as too heavy (user-tested — too
        // dark on pink-day, too light on pink-night). `inactive` is the
        // palette's mid grey-pink for exactly this tier; subtle then body
        // text are the fallbacks, so the line stays readable on any palette
        // that keeps the standard keys.
        text: paletteCell(parsed.colors, ['inactive', 'subtle', 'text']),
  ...
  ctx.inject(['tuiStatus'], statusCtx => {
    ...
    if (typeof status.registerView === 'function') {
      ...
      try {
        ...
        statusCtx.logger.warn(
          `dsh-tui-theme: rich status view registration failed, staying on the scalar line: ${String(error)}`,
        )
      }
    }
    ...
    statusCtx.on('session/event', (session, event) => {
      ...
      if (type === 'turn/start' || type === 'turn/end') render()
    })
