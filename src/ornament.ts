...
function capToCells(text: string, maxCells: number): string {
  let out = ''
  let cells = 0
  for (const ch of text) {
 ...
 */
export function parseOrnamentDraft(text: string): OrnamentDraft | undefined {
  const trimmed = text.trim()
  if (trimmed === '') return { kind: 'clear' }
  if (/[\p{C}]/u.test(trimmed)) return undefined
  const cells = displayCells(trimmed)
  if (cells < 1 || cells > 2) return undefined
 ...
 * line.
 */
export function sanitizeOrnament(value: unknown, fallback: string): string {
